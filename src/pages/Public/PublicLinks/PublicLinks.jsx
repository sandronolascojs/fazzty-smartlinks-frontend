...
export default function PublicLinks () {
  ...
  const { link, loading, error } = useGetLink({ slug })

  if (!loading && !error) {
    document.title = `${link.artists.join(', ')} - ${link.name}`
  }
  ...
  if (error) {
    document.title = 'Fazzty - Error 404'
    return (
      <Error404 />
    )
  }
                ...
                <div className='flex flex-col justify-center items-center py-12 w-auto max-h-fit'>
                  <section className='bg-zinc-200 p-5 rounded-md shadow-md w-[350px]'>
                    ...
                    <div className='py-4 border-b border-zinc-300'>
                      ...
                      <p className='font-medium text-zinc-600 text-lg truncate'>{link.artists.join(', ')}</p>
                    </div>
                    <div>
